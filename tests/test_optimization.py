...
positions = np.array([(marker + np.array([0,0, z_offset])) for marker in markers])
...
for i, position in enumerate(positions):
    ...
    for marker_index, marker in enumerate(markers):
        ...
        rays.append(ray)
    synthetic_snapshots.append(Snapshot(position=position, rays=rays, marker_indices=marker_indices))
...
for i, snapshot in modified_snapshots: